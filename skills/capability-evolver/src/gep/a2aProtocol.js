  ...
  return fetch(endpoint, {
    ...
    .catch(function (err) {
      ...
      if (_heartbeatConsecutiveFailures === 3) {
        console.warn(
          "[Heartbeat] 3 consecutive failures. Network issue? Last error: " + err.message,
        );
      } else if (_heartbeatConsecutiveFailures === 10) {
        console.warn(
          "[Heartbeat] 10 consecutive failures. Hub may be unreachable. (" + err.message + ")",
        );
  ...
  sendHelloToHub()
    .then(function (r) {
      if (r.ok) console.log("[Heartbeat] Registered with hub. Node: " + getNodeId());
        ...
        console.warn(
          "[Heartbeat] Hello failed (will retry via heartbeat): " + (r.error || "unknown"),
        );
    })
    .catch(function () {});
