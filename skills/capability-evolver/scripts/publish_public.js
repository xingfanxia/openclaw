...
function publishToClawhub({ skillDir, slug, name, version, changelog, tags, dryRun }) {
  ...
  try {
    const inspect = spawnClawhub(ok.exe, ["inspect", slug, "--version", version], {
      stdio: "ignore",
    });
    if (inspect.status === 0) {
...
function main() {
  ...
  const useBuildOutput =
    String(process.env.PUBLIC_USE_BUILD_OUTPUT || "true").toLowerCase() === "true";
  ...
  const releaseCreate =
    String(process.env.RELEASE_CREATE || "").toLowerCase() === "true" ? true : !releaseSkip;
  ...
  if (!releaseTag && useBuildOutput) {
    try {
      const builtPkg = JSON.parse(
        fs.readFileSync(path.resolve(process.cwd(), outDir, "package.json"), "utf8"),
      );
      ...
      if (releaseTag) {
        ...
        try {
          ...
          if (!dryRun && exists) {
            ...
            run(`git -C "${tmpRepoDir}" tag -a ${releaseTag} -m "${tagMsg.replace(/"/g, '\\"')}"`, {
              dryRun,
            });
