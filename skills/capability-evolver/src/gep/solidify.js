...
function computeBlastRadius({ repoRoot, baselineUntracked }) {
  ...
  if (untracked.ok) {
    ...
    const baselineSet = new Set(
      (Array.isArray(baselineUntracked) ? baselineUntracked : []).map(normalizeRelPath),
    );
...
function checkConstraints({ gene, blast, blastRadiusEstimate, repoRoot }) {
  ...
  if (gene.strategy) {
    ethicsText +=
      (Array.isArray(gene.strategy) ? gene.strategy.join(" ") : String(gene.strategy)) + " ";
  }
...
function runCanaryCheck(opts) {
  ...
  const timeoutMs =
    opts && Number.isFinite(Number(opts.timeoutMs)) ? Number(opts.timeoutMs) : 30000;
...
function applyEpigeneticMarks(gene, envFingerprint, outcomeStatus) {
  ...
  const nodeVersion =
    envFingerprint && envFingerprint.node_version ? String(envFingerprint.node_version) : "";
  const envContext = [platform, arch, nodeVersion].filter(Boolean).join("/") || "unknown";

  // Check if a mark for this context already exists
  const existingIdx = gene.epigenetic_marks.findIndex((m) => m && m.context === envContext);
...
function ensureGene({ genes, selectedGene, signals, intent, dryRun }) {
  ...
  if (res && res.selected)
    return { gene: res.selected, created: false, reason: "reselected_from_existing" };
...
function solidify({ intent, summary, dryRun = false, rollbackOnFailure = true } = {}) {
  ...
  const appliedLessons =
    lastRun && Array.isArray(lastRun.applied_lessons) ? lastRun.applied_lessons : [];
    ...
    try {
      if (selectedCapsuleId) {
        ...
        prevCapsule = Array.isArray(list)
          ? list.find((c) => c && c.type === "Capsule" && String(c.id) === selectedCapsuleId)
          : null;
      }
    } catch (e) {}
  ...
  if (!dryRun && !success) {
    try {
      ...
      console.log(
        "[Solidify] FailedCapsule capture error (non-fatal): " + (e && e.message ? e.message : e),
      );
    }
  }
  ...
  if (!dryRun && capsule && capsule.a2a && capsule.a2a.eligible_to_broadcast) {
    const autoPublish =
      String(process.env.EVOLVER_AUTO_PUBLISH || "true").toLowerCase() !== "false";
    ...
    if (
      ...
      try {
        ...
        if (hubUrl) {
          ...
          var sanitizedEvent =
            event && event.type === "EvolutionEvent" ? sanitizePayload(event) : null;
  ...
  if (!dryRun && !success) {
    ...
    var hasHighInfoFailure =
      (constraintCheck.violations && constraintCheck.violations.length > 0) ||
  ...
  if (!dryRun && success && lastRun && lastRun.active_task_id) {
    const resultAssetId =
      capsule && capsule.asset_id ? capsule.asset_id : capsule && capsule.id ? capsule.id : null;
    if (resultAssetId) {
      try {
        ...
        if (completed && typeof completed.then === "function") {
          completed
            ...
            .catch(function (err) {
              console.log(
                "[TaskComplete] Failed (non-fatal): " + (err && err.message ? err.message : err),
              );
            });
        }
