  ...
  for (let i = 0; i < messages.length; i++) {
    ...
    if (i === lastUserIdx && !updated.includes("Current time:")) {
      ...
      if (formatted) {
        ...
        let header = `[Now: ${dow} ${formatted}`;
        // Append agent/character timezone when configured and different from user timezone
        if (agentTimezone && agentTimezone !== timezone) {
          const agentFormatted = formatZonedTimestamp(new Date(nowMs), {
            timeZone: agentTimezone,
          });
          if (agentFormatted) {
            header += ` | Agent local: ${agentFormatted}`;
          }
        }
        header += "]";
        updated = `${header}\n${updated}`;
