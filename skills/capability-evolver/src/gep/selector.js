...
function computeDriftIntensity(opts) {
  ...
  var genePoolSize =
    opts && Number.isFinite(Number(opts.genePoolSize)) ? Number(opts.genePoolSize) : null;
...
function selectGeneAndCapsule({
  ...
  const bannedGeneIds =
    memoryAdvice && memoryAdvice.bannedGeneIds instanceof Set
      ...
      : new Set();
  const preferredGeneId =
    memoryAdvice && memoryAdvice.preferredGeneId ? memoryAdvice.preferredGeneId : null;
