...
function generateInnovationIdeas() {
  ...
  const categories = {
    ...
    dev: skills.filter(
      (s) =>
        s.startsWith("git-") || s.startsWith("code-") || s.includes("lint") || s.includes("test"),
    ).length,
    media: skills.filter(
      (s) =>
        s.includes("image") || s.includes("video") || s.includes("music") || s.includes("voice"),
    ).length,
    security: skills.filter(
      (s) => s.includes("security") || s.includes("audit") || s.includes("guard"),
    ).length,
    automation: skills.filter(
      (s) => s.includes("auto-") || s.includes("scheduler") || s.includes("cron"),
    ).length,
    data: skills.filter(
      (s) => s.includes("db") || s.includes("store") || s.includes("cache") || s.includes("index"),
    ).length,
  };
