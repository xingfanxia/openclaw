...
describe("session-seed", () => {
  ...
  describe("buildSeedContextPrefix", () => {
    it("returns formatted string with seed markers for valid session", () => {
      ...
      expect(result).toContain("[End of previous session context]");
    });
    ...
    it("returns null for missing file", () => {
      const result = buildSeedContextPrefix({
        oldSessionFile: "/nonexistent/path.jsonl",
        messageCount: 50,
      });
    ...
    it("respects messageCount limit", () => {
      ...
      writeJsonl(file, [
        ...
        makeMessage("assistant", "Third reply"),
      ]);
      ...
      expect(result).toContain("Assistant: Third reply");
    });
  });
});
