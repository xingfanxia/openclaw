...
function collectDistillationData() {
  ...
  successCapsules.forEach(function (c) {
    ...
    g.total_count += 1;
    g.total_score +=
      c.outcome && Number.isFinite(Number(c.outcome.score)) ? Number(c.outcome.score) : 0.8;
...
function analyzePatterns(data) {
  ...
  var report = {
    ...
    success_rate:
      data.allCapsules.length > 0 ? data.successCapsules.length / data.allCapsules.length : 0,
  };
    ...
    if (ch === "{") {
      ...
      if (depth === 0 && buffer.length > 2) {
        try {
          ...
          if (obj && typeof obj === "object" && obj.type === "Gene") return obj;
        } catch (e) {}
        buffer = "";
      }
      if (depth < 0) depth = 0;
...
function runDistillation() {
  ...
  if (state.last_data_hash === data.dataHash) {
    console.log(
      "[Distiller] Data unchanged since last distillation (hash: " + data.dataHash + "). Skipping.",
    );
