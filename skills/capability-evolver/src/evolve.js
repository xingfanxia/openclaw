...
const IS_RANDOM_DRIFT =
  ARGS.includes("--drift") || String(process.env.RANDOM_DRIFT || "").toLowerCase() === "true";
...
function getMutationDirective(logContent) {
  ...
  const errorMatches =
    logContent.match(/\[ERROR|Error:|Exception:|FAIL|Failed|"isError":true/gi) || [];
...
function writeDormantHypothesis(data) {
  try {
    ...
    console.log(
      "[DormantHypothesis] Write failed (non-fatal): " + (e && e.message ? e.message : e),
    );
  }
}
...
function getRecentActiveSessionCount(windowMs) {
  try {
    ...
    return fs
      ...
      .filter((f) => {
        try {
          return now - fs.statSync(path.join(AGENT_SESSIONS_DIR, f)).mtimeMs < w;
        } catch (_) {
          return false;
        }
      }).length;
...
async function run() {
  ...
  if (process.platform !== "win32") {
    try {
      const _psRace = require("child_process")
        .execSync('ps aux | grep "evolver_hand_" | grep "openclaw.*agent" | grep -v grep', {
          encoding: "utf8",
          timeout: 5000,
          stdio: ["ignore", "pipe", "ignore"],
        })
  ...
  if (sysLoad.load1m > LOAD_MAX) {
    console.log(
      `[Evolver] System load ${sysLoad.load1m.toFixed(2)} exceeds max ${LOAD_MAX}. Backing off ${QUEUE_BACKOFF_MS}ms.`,
    );
    writeDormantHypothesis({
  ...
  if (bridgeEnabled && loopMode) {
    try {
      ...
      if (lastRun && lastRun.run_id) {
        const pending =
          !lastSolid || !lastSolid.run_id || String(lastSolid.run_id) !== String(lastRun.run_id);
        if (pending) {
          writeDormantHypothesis({
            ...
            personality_state:
              lastRun && lastRun.personality_state ? lastRun.personality_state : null,
            run_id: lastRun.run_id,
          });
          const raw =
            process.env.EVOLVE_PENDING_SLEEP_MS || process.env.EVOLVE_MIN_INTERVAL || "120000";
  ...
  try {
    ...
    if (recent.length >= REPAIR_LOOP_THRESHOLD) {
      const allRepairFailed = recent.every(
        (e) => e && e.intent === "repair" && e.outcome && e.outcome.status === "failed",
      );
      if (allRepairFailed) {
  ...
  if (
    ...
    if (injected > 0) {
      console.log(
        "[DormantHypothesis] Injected " + injected + " signal(s) from previous interrupted cycle.",
      );
    }
  }
  ...
  const recentErrorMatches =
    recentMasterLog.match(/\[ERROR|Error:|Exception:|FAIL|Failed|"isError":true/gi) || [];
  ...
  const selectedBy =
    memoryAdvice && memoryAdvice.preferredGeneId ? "memory_graph+selector" : "selector";
  ...
  const tailAvgScore =
    tail.length > 0
      ? tail.reduce(
          (acc, e) =>
            acc +
            (e && e.outcome && Number.isFinite(Number(e.outcome.score))
  ...
  try {
    ...
    prevState.last_run = {
      ...
      source_type:
        hubHit && hubHit.hit ? (hubHit.mode === "direct" ? "reused" : "reference") : "generated",
  ...
  const recentHistorySummary = (() => {
    ...
    const lines = last8.map((evt, idx) => {
      ...
      const gene =
        Array.isArray(evt.genes_used) && evt.genes_used.length ? evt.genes_used[0] : "none";
...
`.trim();
  ...
  const emitThought =
    String(process.env.EVOLVE_EMIT_THOUGHT_PROCESS || "").toLowerCase() === "true";
  if (emitThought) {
