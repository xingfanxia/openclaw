...
export function maybeHandleQueueDirective(params: {
  ...
  if (wantsStatus) {
    ...
    return {
      text: withOptions(
        ...
        "modes steer, followup, collect, steer+backlog, interrupt, parallel; debounce:<ms|s|m>, cap:<n>, drop:old|new|summarize",
      ),
    };
  }
  ...
  if (queueModeInvalid || queueDebounceInvalid || queueCapInvalid || queueDropInvalid) {
    ...
    if (queueModeInvalid) {
      errors.push(
        `Unrecognized queue mode "${directives.rawQueueMode ?? ""}". Valid modes: steer, followup, collect, steer+backlog, interrupt, parallel.`,
      );
    }
