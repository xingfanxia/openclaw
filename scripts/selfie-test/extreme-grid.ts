...
const CAPTIONS_PATH = "/home/xingfanxia/tmp/extreme-grid-captions.json";
const OUTPUT_BASE = "/home/xingfanxia/tmp/extreme-grid";
...
async function writeSummary(runDir: string, results: Result[]): Promise<void> {
  const lines: string[] = [];
  lines.push(`# Extreme-Spicy Grid Rescue Experiment (B2)\n`);
...
async function main(): Promise<void> {
  ...
  if (!geminiKey) {
    console.error("[B2] GEMINI_API_KEY not set");
    process.exit(1);
  }
  if (!doubaoKey) {
    console.error("[B2] ARK_API_KEY not set");
    process.exit(1);
  }
  ...
  console.log(
    `[B2] ${cases.length} cases (${captions.length} caps × 2 count × 2 provider), concurrency ${CONCURRENCY}`,
  );
  console.log(`[B2] Run dir: ${runDir}`);
  ...
  const block = results.filter((r) => r.status === "block").length;
  const fail = results.filter((r) => r.status === "fail").length;
  console.log(`[B2] Done in ${elapsed}s. Pass ${pass} / Block ${block} / Fail ${fail}`);
  console.log(`[B2] Summary: ${path.join(runDir, "summary.md")}`);
}
