...
export async function acquireSessionWriteLock(params: {
  ...
  while (Date.now() - startedAt < timeoutMs) {
    ...
    try {
      ...
      const inspected = inspectLockPayload(payload, staleMs, nowMs);
      // Enhanced PID recycle detection using Linux /proc/pid/stat startTime.
      ...
      if (!inspected.stale && inspected.pidAlive && inspected.pid != null && process.platform === "linux") {
        ...
        if (currentStartTime != null) {
          ...
          } else if (inspected.pid === process.pid) {
            const createdAtMs = inspected.createdAt ? Date.parse(inspected.createdAt) : Number.NaN;
            if (Number.isFinite(createdAtMs)) {
              const processStartedAt = Date.now() - process.uptime() * 1000;
              recycledPid = createdAtMs < processStartedAt - 1000;
            }
          }
        }
      }
